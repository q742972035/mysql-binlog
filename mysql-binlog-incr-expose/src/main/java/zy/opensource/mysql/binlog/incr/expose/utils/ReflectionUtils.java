...
public abstract class ReflectionUtils {
	 ...
	 */
	private static final Map<Class<?>, Method[]> declaredMethodsCache = new ManagedConcurrentWeakHashMap<>(256);

	/**
	 * Cache for {@link Class#getDeclaredFields()}, allowing for fast iteration.
	 */
	private static final Map<Class<?>, Field[]> declaredFieldsCache = new ManagedConcurrentWeakHashMap<>(256);

	private static final Map<ConstructorWrap, Constructor<?>> declaredConstructorsCache = new ManagedConcurrentWeakHashMap<>(256);
	// Exception handling
	...
	public static <T> Constructor<T> accessibleConstructor(Class<T> clazz, Class<?>... parameterTypes)
		...
		ConstructorWrap wrap = new ConstructorWrap(clazz,parameterTypes);
		Constructor ctor;
		if ((ctor=declaredConstructorsCache.get(wrap))!=null){
			return ctor;
		}
		ctor = clazz.getDeclaredConstructor(parameterTypes);
		makeAccessible(ctor);
		declaredConstructorsCache.put(wrap,ctor);
		return ctor;
	}


	public static class ConstructorWrap{
		Class type;
		Class<?>[] parameterTypes;

		public ConstructorWrap(Class type, Class<?>[] parameterTypes) {
			this.type = type;
		...
		public boolean equals(Object o) {
			...
			ConstructorWrap wrap = (ConstructorWrap) o;
			return Objects.equals(type, wrap.type) &&
					Arrays.equals(parameterTypes, wrap.parameterTypes);
		}
