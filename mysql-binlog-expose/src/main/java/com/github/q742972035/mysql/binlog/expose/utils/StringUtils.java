...
public class StringUtils {
     ...
     * @return
     */
    public static boolean compareFormerLower(String src, int len, String former) {
        if (src == null || src.length() < len) {
            return false;
        }
        String lower = src.substring(0, len).toLowerCase();
        return former.equals(lower);
    }
    ...
    public static String generateKey(String...keys){
        int hash = ObjectUtils.nullSafeHashCode(keys);
        StringBuilder keyBuider = new StringBuilder();
        for (String key : keys) {
            keyBuider.append(key).append(hash);
        }
